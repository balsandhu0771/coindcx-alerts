...
def fetch_ohlcv(symbol_base, interval='4h', limit=25):
    ...
    try:
        ...
        res = requests.get(BYBIT_KLINE_URL, params=params, timeout=5)
        if res.status_code == 200:
    ...
    try:
        ...
        res = requests.get(COINDCX_CANDLES_URL, params=params, timeout=5)
        if res.status_code == 200:
            data = res.json()
...
def extract_15m_levels(symbol_base, setup_type, h_ref, l_ref):
    try:
        ...
        # 16 completed 15m bars of the 4H sweep candle session
...
def check_liquidity_sweep(symbol_base):
    try:
        ...
        # [-3] = Reference Candle, [-2] = Trigger Candle (just closed)
        c_ref = ohlcv_4h[-3]
        c_trig = ohlcv_4h[-2]
...
def run_full_scan():
    ...
    try:
        ...
        source_tag = f"{os.environ.get('RENDER_SERVICE_NAME', 'RENDER')}-{os.environ.get('RENDER_INSTANCE_ID', 'DEFAULT')[:6]}"

        msg = f"⚡ *4H LIQUIDITY SWEEP SCAN COMPLETE* ⚡\n"
        msg += f"━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"📊 *Pairs Evaluated (>= $5M Vol):* `{len(symbols)}`\n"
