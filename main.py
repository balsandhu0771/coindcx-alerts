import os
import time
import threading
from datetime import datetime, timezone
import requests
import ccxt
from flask import Flask, jsonify

# =====================================================================
# 1. CONFIGURATION & CREDENTIALS
# =====================================================================
TELEGRAM_BOT_TOKEN = "8642933768:AAH3afnXGmaAplHDar9u4uwJ5IZz0M7y7fs"
TELEGRAM_CHAT_IDS = [7203290966, 630462102]

# Safe Port Parsing for Render
raw_port = os.environ.get("PORT", "8080")
try:
    PORT = int(raw_port)
except Exception:
    PORT = 8080

# Initialize CCXT Binance USDⓈ-M Futures Client
exchange = ccxt.binanceusdm({
    'enableRateLimit': True,
    'options': {
        'defaultType': 'future',
        'adjustForTimeDifference': True
    }
})

# In-memory storage for active setups being monitored on 15m candles
active_watchlists = {}  # {symbol: setup_data_dict}
processed_sweeps = set() # (symbol, timestamp) tracker to avoid duplicate watchlist adds

# =====================================================================
# 2. TELEGRAM DISPATCHER
# =====================================================================
def send_telegram_alert(message):
    """Dispatches a formatted Markdown message to all configured Chat IDs."""
    for chat_id in TELEGRAM_CHAT_IDS:
        try:
            url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
            payload = {
                "chat_id": chat_id,
                "text": message,
                "parse_mode": "Markdown",
                "disable_web_page_preview": True
            }
            res = requests.post(url, json=payload, timeout=10)
            if res.status_code != 200:
                print(f"[TELEGRAM ERROR] Status {res.status_code}: {res.text}")
        except Exception as e:
            print(f"[TELEGRAM EXCEPTION] Failed sending to {chat_id}: {e}")

# =====================================================================
# 3. 15-MINUTE MARKET STRUCTURE SHIFT (MSS) EXTRACTION
# =====================================================================
def extract_15m_levels(symbol, setup_type, h_ref, l_ref, c_trig_high, c_trig_low):
    """
    Extracts the highest swing high or lowest swing low within the 4H sweep window
    and enforces boundary constraints to validate the Market Structure Shift (MSS).
    """
    try:
        # Fetch last 20 candles of 15m
        ohlcv_15m = exchange.fetch_ohlcv(symbol, timeframe='15m', limit=20)
        if len(ohlcv_15m) < 16:
            return None

        # Closed 15m candles covering the 4-hour sweep period
        window_15m = ohlcv_15m[-17:-1]

        if setup_type == "LONG":
            # 1. Identify the absolute sweep low (L_min)
            l_min = min(c[3] for c in window_15m)
            min_idx = [i for i, c in enumerate(window_15m) if c[3] == l_min][0]

            # 2. Identify the closest 15m swing high prior to / at this low
            h_mss = None
            for i in range(min_idx, -1, -1):
                c = window_15m[i]
                if h_mss is None or c[2] > h_mss:
                    h_mss = c[2]

            if h_mss is None:
                return None

            # 3. Boundary Check: Entry high must sit logically above the reference 4H low
            if h_mss <= l_ref:
                print(f"[{symbol}] Filtered: 15m MSS ({h_mss}) <= Reference Low ({l_ref})")
                return None

            # Calculate True Target & Risk:Reward
            target = h_ref
            risk = h_mss - l_min
            reward = target - h_mss

            if risk <= 0 or reward <= 0:
                return None

            rr_ratio = round(reward / risk, 2)

            return {
                "l_min": l_min,
                "h_mss": h_mss,
                "l_ref": l_ref,
                "h_ref": h_ref,
                "entry": h_mss,
                "stop_loss": l_min,
                "target": target,
                "rr_ratio": rr_ratio
            }

        elif setup_type == "SHORT":
            # 1. Identify the absolute sweep high (H_max)
            h_max = max(c[2] for c in window_15m)
            max_idx = [i for i, c in enumerate(window_15m) if c[2] == h_max][0]

            # 2. Identify the closest 15m swing low prior to / at this high
            l_mss = None
            for i in range(max_idx, -1, -1):
                c = window_15m[i]
                if l_mss is None or c[3] < l_mss:
                    l_mss = c[3]

            if l_mss is None:
                return None

            # 3. Boundary Check: Entry low must sit logically below the reference 4H high
            if l_mss >= h_ref:
                print(f"[{symbol}] Filtered: 15m MSS ({l_mss}) >= Reference High ({h_ref})")
                return None

            # Calculate True Target & Risk:Reward
            target = l_ref
            risk = h_max - l_mss
            reward = l_mss - target

            if risk <= 0 or reward <= 0:
                return None

            rr_ratio = round(reward / risk, 2)

            return {
                "h_max": h_max,
                "l_mss": l_mss,
                "h_ref": h_ref,
                "l_ref": l_ref,
                "entry": l_mss,
                "stop_loss": h_max,
                "target": target,
                "rr_ratio": rr_ratio
            }

    except Exception as e:
        print(f"[{symbol}] 15m Level extraction error: {e}")
        return None

# =====================================================================
# 4. 4H LIQUIDITY SWEEP DETECTION
# =====================================================================
def check_liquidity_sweep(symbol):
    """
    Evaluates closed 4H candles to detect valid bullish/bearish liquidity sweeps.
    """
    try:
        ohlcv_4h = exchange.fetch_ohlcv(symbol, timeframe='4h', limit=5)
        if len(ohlcv_4h) < 4:
            return None

        # [-3] = Reference 4H Candle
        # [-2] = Trigger 4H Candle (Just closed)
        # [-1] = Current forming live 4H candle
        c_ref = ohlcv_4h[-3]
        c_trig = ohlcv_4h[-2]

        h_ref, l_ref = c_ref[2], c_ref[3]
        h_trig, l_trig, c_trig_close = c_trig[2], c_trig[3], c_trig[4]

        # 1. Long Sweep Check (Took Low, Closed Inside, Did not sweep High)
        long_sweep = (l_trig < l_ref) and (c_trig_close > l_ref) and (h_trig <= h_ref)

        # 2. Short Sweep Check (Took High, Closed Inside, Did not sweep Low)
        short_sweep = (h_trig > h_ref) and (c_trig_close < h_ref) and (l_trig >= l_ref)

        if not (long_sweep or short_sweep):
            return None

        setup_type = "LONG" if long_sweep else "SHORT"

        # Extract precise 15m levels & boundaries
        levels = extract_15m_levels(symbol, setup_type, h_ref, l_ref, h_trig, l_trig)
        if not levels:
            return None

        # Determine if immediate 15m break already occurred
        ohlcv_15m = exchange.fetch_ohlcv(symbol, timeframe='15m', limit=10)
        recent_15m = ohlcv_15m[-5:-1]

        already_mss = False
        if setup_type == "LONG":
            already_mss = any(c[4] > levels['h_mss'] for c in recent_15m)
        else:
            already_mss = any(c[4] < levels['l_mss'] for c in recent_15m)

        return {
            "symbol": symbol,
            "type": setup_type,
            "levels": levels,
            "already_mss": already_mss,
            "candle_timestamp": c_trig[0],
            "detected_at": datetime.now(timezone.utc)
        }

    except Exception as e:
        print(f"Error checking {symbol}: {e}")
        return None

# =====================================================================
# 5. MARKET SCAN WORKERS
# =====================================================================
def run_full_scan():
    """Scans the top 120 USDT perpetual pairs for fresh 4H sweeps."""
    print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Launching Full 4H Sweep Scan...")
    try:
        markets = exchange.load_markets()
        symbols = [
            s for s, m in markets.items()
            if m.get('active', True)
            and m.get('quote') == 'USDT'
            and (m.get('linear', False) or m.get('swap', False) or m.get('contract', False))
        ]

        # Scan top 120 most liquid contracts
        symbols = symbols[:120]
        found_setups = []

        for sym in symbols:
            res = check_liquidity_sweep(sym)
            if res:
                found_setups.append(res)
                active_watchlists[sym] = res
            time.sleep(0.04)

        # Build Full Structured Telegram Alert
        msg = f"⚡ *4H LIQUIDITY SWEEP SCAN COMPLETE* ⚡\n"
        msg += f"━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"📊 *Pairs Evaluated:* `{len(symbols)}`\n"
        msg += f"🎯 *Setups Identified:* `{len(found_setups)}`\n"
        msg += f"⏰ *Scan Time (UTC):* `{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}`\n"

        if found_setups:
            for s in found_setups:
                sym_clean = s['symbol'].split(':')[0]
                lvl = s['levels']
                status_icon = "🟢" if s['type'] == "LONG" else "🔴"
                msg += f"\n━━━━━━━━━━━━━━━━━━━━\n"
                msg += f"{status_icon} *{sym_clean}* | *{s['type']} SWEEP*\n"
                msg += f"• *Entry Level (15m MSS):* `{lvl['entry']}`\n"
                msg += f"• *Invalidation (SL):* `{lvl['stop_loss']}`\n"
                msg += f"• *Target (4H Liquidity):* `{lvl['target']}`\n"
                msg += f"• *Risk : Reward:* `1 : {lvl['rr_ratio']}`\n"
                msg += f"• *MSS Triggered:* `{'YES (Immediate)' if s['already_mss'] else 'NO (Monitoring 15m)'}`"
        else:
            msg += "\nNo fresh 4H sweeps currently meeting all structural parameters."

        send_telegram_alert(msg)
        print("4H Scan completed and notification broadcasted.")

    except Exception as e:
        print(f"Scan Execution Error: {e}")

def run_15m_check():
    """Monitors the active watchlist on every 15-minute candle close."""
    if not active_watchlists:
        return

    print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Evaluating 15m Watchlist ({len(active_watchlists)} coins)...")
    to_remove = []

    for sym, data in list(active_watchlists.items()):
        try:
            ohlcv = exchange.fetch_ohlcv(sym, timeframe='15m', limit=3)
            last_closed = ohlcv[-2]
            c_close = last_closed[4]
            levels = data['levels']
            sym_clean = sym.split(':')[0]

            if data['type'] == "LONG":
                # Invalidation Check: Price pierced below sweep low
                if last_closed[3] < levels['l_min']:
                    print(f"[{sym}] Setup Invalidated (Broke low).")
                    to_remove.append(sym)
                    continue

                # MSS Break Confirmation: Closed above swing high
                if c_close > levels['h_mss']:
                    send_telegram_alert(
                        f"🚀 *LONG MSS CONFIRMED: {sym_clean}*\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"• *Entry Trigger:* `{levels['entry']}`\n"
                        f"• *Stop Loss:* `{levels['stop_loss']}`\n"
                        f"• *Target (4H High):* `{levels['target']}`\n"
                        f"• *Genuine R:R:* `1 : {levels['rr_ratio']}`\n"
                        f"• *Status:* Confirmed 15m Candle Close"
                    )
                    to_remove.append(sym)

            elif data['type'] == "SHORT":
                # Invalidation Check: Price pierced above sweep high
                if last_closed[2] > levels['h_max']:
                    print(f"[{sym}] Setup Invalidated (Broke high).")
                    to_remove.append(sym)
                    continue

                # MSS Break Confirmation: Closed below swing low
                if c_close < levels['l_mss']:
                    send_telegram_alert(
                        f"🔻 *SHORT MSS CONFIRMED: {sym_clean}*\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"• *Entry Trigger:* `{levels['entry']}`\n"
                        f"• *Stop Loss:* `{levels['stop_loss']}`\n"
                        f"• *Target (4H Low):* `{levels['target']}`\n"
                        f"• *Genuine R:R:* `1 : {levels['rr_ratio']}`\n"
                        f"• *Status:* Confirmed 15m Candle Close"
                    )
                    to_remove.append(sym)

        except Exception as e:
            print(f"Error checking 15m status for {sym}: {e}")

    for sym in to_remove:
        active_watchlists.pop(sym, None)

# =====================================================================
# 6. 24/7 BACKGROUND SCHEDULER
# =====================================================================
def scheduler_loop():
    """Runs continuously and executes scans exactly 1 minute past candle closes."""
    while True:
        now = datetime.now(timezone.utc)
        # Execute 1 minute past every 15-minute close (:01, :16, :31, :46)
        if now.minute in [1, 16, 31, 46] and now.second < 20:
            # 4H Binance Candle Close Windows (00:00, 04:00, 08:00, 12:00, 16:00, 20:00 UTC)
            if now.hour in [0, 4, 8, 12, 16, 20] and now.minute == 1:
                run_full_scan()
            else:
                run_15m_check()
            time.sleep(60)
        time.sleep(5)

# =====================================================================
# 7. FLASK WEB SERVER & DIAGNOSTIC ENDPOINTS
# =====================================================================
app = Flask(__name__)

@app.route('/')
def home():
    return jsonify({
        "status": "online",
        "service": "crypto-alerts-24-7",
        "active_watchlists_count": len(active_watchlists),
        "server_time_utc": datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
    })

@app.route('/trigger-scan')
def trigger_scan():
    threading.Thread(target=run_full_scan, daemon=True).start()
    return "Manual 4H Sweep + 15m MSS scan started! Check Telegram in 2 minutes."

@app.route('/debug-token/<path:symbol>')
def debug_token_endpoint(symbol):
    clean = symbol.upper().replace('-', '/').split(':')[0]
    if not clean.endswith('/USDT') and not clean.endswith('USDT'):
        clean += '/USDT'
    elif clean.endswith('USDT') and '/' not in clean:
        clean = clean.replace('USDT', '/USDT')

    formatted = f"{clean}:USDT"

    try:
        ohlcv_4h = exchange.fetch_ohlcv(formatted, timeframe='4h', limit=6)
        c_ref = ohlcv_4h[-3]
        c_trig = ohlcv_4h[-2]

        h_ref, l_ref = c_ref[2], c_ref[3]
        h_trig, l_trig, c_trig_close = c_trig[2], c_trig[3], c_trig[4]

        long_sweep = bool((l_trig < l_ref) and (c_trig_close > l_ref) and (h_trig <= h_ref))
        short_sweep = bool((h_trig > h_ref) and (c_trig_close < h_ref) and (l_trig >= l_ref))

        eval_result = check_liquidity_sweep(formatted)

        return jsonify({
            "symbol": formatted,
            "4h_reference_candle_3": {"high": h_ref, "low": l_ref},
            "4h_trigger_candle_2": {"high": h_trig, "low": l_trig, "close": c_trig_close},
            "conditions_met": {
                "long_sweep": long_sweep,
                "short_sweep": short_sweep
            },
            "passed_full_evaluation": bool(eval_result),
            "evaluation_details": eval_result
        })
    except Exception as e:
        return jsonify({"error": str(e)})

# =====================================================================
# 8. PROCESS ENTRYPOINT
# =====================================================================
if __name__ == '__main__':
    t = threading.Thread(target=scheduler_loop, daemon=True)
    t.start()
    print("=== 24/7 Market Monitor Initialized Successfully ===")
    app.run(host='0.0.0.0', port=PORT)
