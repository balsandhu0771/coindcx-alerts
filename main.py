...
processed_sweeps = (
    set()
)  # Format: set of (symbol, 4h_candle_timestamp) to avoid repeat alerts
  ...
  for chat_id in TELEGRAM_CHAT_IDS:
    try:
      ...
      if res.status_code != 200:
        logger.error(
            f"[TELEGRAM FAIL] Chat ID: {chat_id} | Status: {res.status_code} |"
            f" Body: {res.text}"
        )
      else:
      ...
      logger.error(
          f"[TELEGRAM EXCEPTION] Failed to dispatch message to {chat_id}: {e}"
      )


# =====================================================================
# 3. COMPLETE FUTURES PERPETUALS DISCOVERY ENGINE
# =====================================================================
def get_all_futures_symbols():
  """Loads every single active USDT-settled perpetual futures contract directly from Binance."""
  try:
    logger.info(
        "Loading complete universe of Binance USDⓈ-M perpetual contracts..."
    )
    ...
    for symbol, m in markets.items():
      ...
      # CCXT market structure checking for linear swap contracts
      is_usdt_settled = (m.get("quote") == "USDT") or (
          m.get("settle") == "USDT"
      )
      is_perpetual = (
          m.get("swap", False)
      ...
      if is_usdt_settled and is_perpetual:
        # Standardize format for CCXT OHLCV fetching
    ...
    logger.info(
        f"Loaded {len(unique_symbols)} active perpetual markets to evaluate."
    )
...
def extract_15m_levels(symbol, setup_type, h_ref, l_ref):
  ...
  and enforces strict boundary constraints to validate the Market Structure
  Shift.
  """
  try:
    # Fetch last 32 15m candles
...
def check_liquidity_sweep(symbol):
  ...
  try:
    ...
    # [-3] = Reference Candle, [-2] = Trigger Candle (Just closed), [-1] = Forming candle
    c_ref = ohlcv_4h[-3]
    c_trig = ohlcv_4h[-2]
...
# =====================================================================
def run_full_scan():
  """Executes full market scan over all active perpetual pairs and dispatches alerts."""
  ...
  try:
    ...
    for sym in symbols:
      ...
      time.sleep(0.02)  # Fast pacing without rate limits
    ...
    if found_setups:
      for s in found_setups:
        ...
        msg += (
            "• *MSS Triggered:*"
            f" `{'YES (Immediate)' if s['already_mss'] else 'NO (Monitoring 15m)'}`"
        )
    else:
    ...
    logger.info(
        f"Scan completed in {elapsed}s with {len(found_setups)} setups"
        " dispatched."
    )
...
def run_15m_check():
  ...
  logger.info(
      f"Evaluating 15m Watchlist ({len(active_watchlists)} active setups)..."
  )
  to_remove = []
...
def trigger_scan():
  send_telegram_alert(
      "⚡ *Manual scan initiated via Webhook...* Evaluating all Perpetual pairs"
      " now."
  )
...
if __name__ == "__main__":
  send_telegram_alert(
      ...
      "• *Coverage:* Complete Active Futures Perpetuals Catalog (280+ Pairs)\n"
